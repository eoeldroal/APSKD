...
STUDENT_MODEL_PATH="${STUDENT_MODEL_PATH:-${REPO_ROOT}/checkpoints/Qwen3-1.7B}"
...
ENGINE_MAX_NUM_SEQS="${ENGINE_MAX_NUM_SEQS:-512}"

TRAIN_PROMPT_BSZ="${TRAIN_PROMPT_BSZ:-96}"
PREFETCH_LIMIT="${PREFETCH_LIMIT:-0}"
PREFETCH_WORKER_TARGET="${PREFETCH_WORKER_TARGET:-0}"
STUDENT_MAX_TOKEN_LEN_PER_GPU="${STUDENT_MAX_TOKEN_LEN_PER_GPU:-12000}"
...
SKD_MAX_CHUNKS="${SKD_MAX_CHUNKS:-512}"
...
EXP_NAME="${EXPERIMENT_NAME:-base/fsdp/student-Qwen3-1.7B/teacher-Qwen3-32B-FP8/train-nemotron-cascade-rl-math/loss-${DISTILLATION_LOSS_MODE}/pg-${USE_POLICY_GRADIENT}/8gpu-4x4-async-skd-tool-longctx16k-b${TRAIN_PROMPT_BSZ}-prefetch${PREFETCH_LIMIT}-wtarget${PREFETCH_WORKER_TARGET}-chunk${SKD_CHUNK_SIZE}-topk${SKD_VERIFY_TOP_K}-maxchunks${SKD_MAX_CHUNKS}}"
...
VERL_ASYNC_SKD_EVENT_LOG="${VERL_ASYNC_SKD_EVENT_LOG:-${REPO_ROOT}/logs/async_skd_events_live.jsonl}"
MONKEY_PATCH_TIMING="${MONKEY_PATCH_TIMING:-1}"
MONKEY_PATCH_TIMING_SLOW_MS="${MONKEY_PATCH_TIMING_SLOW_MS:-500}"
MONKEY_PATCH_GPU_UTIL="${MONKEY_PATCH_GPU_UTIL:-1}"
MONKEY_PATCH_GPU_UTIL_SAMPLE_MS="${MONKEY_PATCH_GPU_UTIL_SAMPLE_MS:-200}"
SKD_DEBUG="${SKD_DEBUG:-1}"
...
ROLLOUT=(
    ...
    +actor_rollout_ref.rollout.agent.async_skd_prefetch_worker_target=$PREFETCH_WORKER_TARGET
...
cd "${REPO_ROOT}"
mkdir -p "$(dirname "${VERL_ASYNC_SKD_EVENT_LOG}")"

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}"
export HYDRA_FULL_ERROR=1
export VERL_ASYNC_SKD_EVENT_LOG
