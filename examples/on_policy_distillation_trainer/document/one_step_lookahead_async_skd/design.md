...
이 문서의 목표는 **intra-step tail filling**이다. Step `k` rollout 내부에서 long trajectory가 tail을 만들 때, 이미 자기 몫을 끝낸 student-teacher pair가 제한된 수의 step `k+1` sample을 미리 생성한다. 따라서 핵심은 batch-level pipeline이 아니라 sample-level scheduling, bounded lookahead, committed-unit-boundary carryover다.
...
이 경로는 SKD chunk, teacher verification, tool execution 중간에는 반환하지 않는다. `ASSISTANT_GEN_CHUNK`, `ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT`, `ASSISTANT_GEN_CHUNK_WITH_INTERACTION_RESULT` 중 export 가능한 boundary에서만 partial carry-over를 만든다.
...
- Lookahead sample이 terminal까지 끝나지 않으면 다음 committed-unit boundary까지만 진행한 뒤 pause한다.
...
Lookahead sample 중 terminal까지 끝나지 않았지만 committed-unit boundary에서 멈춘 sample이다. Step `k+1`에서 current version으로 이어서 생성한다.

### 2.7 Last Committed Unit

`skd_last_committed_unit`은 async scheduler 전용 상태명이 아니라, trajectory가 어디까지 완전히 반영되었는지를 나타내는 일반 metadata다.

이 문서에서는 다음 unit만 partial carry-over로 저장 가능한 committed unit으로 인정한다.

- `ASSISTANT_GEN_CHUNK`: student chunk 생성, teacher verification, first-rejection commit, teacher row append가 끝난 상태
- `ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT`: assistant tool-call이 확정되고, full tool execution, full tool response append, `response_mask=0`, dummy teacher rows append, teacher prompt stream update가 모두 끝난 상태
- `ASSISTANT_GEN_CHUNK_WITH_INTERACTION_RESULT`: full interaction/user response append가 끝난 상태

Mid-tool response, teacher verification 전, chunk commit 전, serialization 중간은 committed unit이 아니다.
...
`version_span`은 하나의 trajectory 안에서 관측된 `max_rollout_version - min_rollout_version`이다. Weight sync가 committed-unit boundary에서만 적용되더라도, old prefix와 new suffix가 섞인 resumed sample은 `version_span=1`이 될 수 있다.
...
Step `k`의 student parameter를 `theta_k`라고 하자. Student version `theta`가 만드는 committed-unit trajectory 분포를 `P_theta`라고 둔다.
...
Mid-tool interruption이 발생하면 다음이 깨질 수 있다.

- `messages`와 `prompt_ids`의 일관성
- `response_mask`와 `teacher_ids_list`의 길이 정렬
- Hermes/GPT-OSS tool format
- teacher prompt stream과 student prompt stream의 동기화
- multimodal image/video reference alignment
- tool reward and turn score accounting

따라서 tool result는 반드시 all-or-none atomic unit이다. Carry-over snapshot은 `ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT` 이후 상태를 선호해야 한다. `ASSISTANT_GEN_CHUNK`도 prefix 관점에서는 저장 가능하지만, 해당 assistant prefix가 이미 완성된 tool call을 포함한다면 scheduler는 가능하면 tool macro-step까지 닫은 뒤 pause하는 것이 안전하다.
...
```text
SkdCommittedUnit:
  resume 가능한 committed-unit boundary 집합.
...
| `PAUSED_CARRYOVER` | Lookahead sample paused at a committed-unit boundary |
...
`skd_last_committed_unit`은 이 outer state를 대체하지 않는다. 또한 엄밀히 말해 outer state의 sub-state도 아니다. 이 값은 각 handler가 실행을 마친 뒤, trajectory에 마지막으로 완전히 반영된 atomic unit을 기록하는 metadata다.

### 10.3 Handler-Internal Atomic Units

각 outer state handler 내부에서 실제로 commit되는 atomic unit은 다음과 같다.

```text
PENDING
  _handle_pending_state
    prompt streams initialized
    no committed response unit
    no skd_last_committed_unit update
    -> GENERATING
...
PROCESSING_TOOLS
  _handle_processing_tools_state
    ...
    alignment is asserted
    if a tool-result span was appended:
      skd_last_committed_unit = ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT
...
INTERACTING
  _handle_interacting_state
    ...
    alignment is asserted
    if an interaction span was appended:
      skd_last_committed_unit = ASSISTANT_GEN_CHUNK_WITH_INTERACTION_RESULT
    -> GENERATING | TERMINATED
```

이 관점에서 `ASSISTANT_GEN_CHUNK`, `ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT`, `ASSISTANT_GEN_CHUNK_WITH_INTERACTION_RESULT`는 “sub-state”라기보다 **last committed unit boundary**다. 같은 outer next state가 `GENERATING`이어도 마지막 committed unit은 다를 수 있다.

예를 들어:

```text
case A:
  previous outer state = GENERATING
  handler completed assistant chunk
  next outer state = GENERATING or PROCESSING_TOOLS
  skd_last_committed_unit = ASSISTANT_GEN_CHUNK

case B:
  previous outer state = PROCESSING_TOOLS
  handler completed tool result append
  next outer state = GENERATING
  skd_last_committed_unit = ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT
```

두 경우 모두 이후에 assistant generation으로 이어질 수 있지만, committed prefix의 의미가 다르다. `case B`는 tool/environment observation까지 prompt에 들어간 상태다.

Allowed committed-unit boundaries:

| Outer handler | Atomic unit completed | `skd_last_committed_unit` |
|---|---|---|
| `_handle_generating_state` | assistant SKD chunk commit | `ASSISTANT_GEN_CHUNK` |
| `_handle_processing_tools_state` | tool result append and dummy row alignment | `ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT` |
| `_handle_interacting_state` | interaction result append and dummy row alignment | `ASSISTANT_GEN_CHUNK_WITH_INTERACTION_RESULT` |

Forbidden interruption points:

- inside student chunk generation
- after student proposal but before teacher verification
- inside teacher verification
- after rejection decision but before commit
- after assistant tokens append but before teacher rows append
- during tool execution
- during tool response serialization
- after `response_mask` append but before dummy teacher row append

### 10.4 Partial Carry-Over Interpretation

For async SKD, the scheduler should interpret `skd_last_committed_unit` as follows.
...
```text
1. finish the currently running atomic unit.
2. assert alignment.
3. record skd_last_committed_unit.
4. export partial state only if skd_last_committed_unit is in RESUMABLE_COMMITTED_UNITS.
```

`AgentState.TERMINATED` is not a committed unit. It is the outer completion state. A terminated sample is not resumed; it is either promoted into the current train batch or consumed as a completed sample.

Special case:

```text
skd_last_committed_unit = ASSISTANT_GEN_CHUNK
next outer state = PROCESSING_TOOLS
```

This means the assistant prefix already contains a parsed tool call, but the tool result is not yet appended. The prefix is token-aligned and teacher-aligned, but semantically the environment observation is still pending. The scheduler should prefer advancing through `_handle_processing_tools_state` and pausing at `ASSISTANT_GEN_CHUNK_WITH_TOOL_RESULT`. If tool execution cannot be safely performed, the scheduler may pause at `ASSISTANT_GEN_CHUNK` only with explicit pending-tool metadata.
...
| `BARRIER_DRAIN` | Current LT finishes, lookahead tasks stop at next committed unit boundary |
...
Trainer가 update를 끝내면 student rollout engines에만 weight sync를 건다. Sync는 active sample의 committed-unit boundary에서 적용한다. 이 구조는 GPU utilization을 높이지만, 안전한 work가 없으면 의도적으로 idle을 허용한다.
...
Promote every finished lookahead sample.
Pause unfinished samples only at atomic committed-unit boundaries.
